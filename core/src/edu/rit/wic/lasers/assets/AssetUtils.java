...
/**
 * General utilities for dealing with {@link Asset assets}. Mostly dealing with operations using {@link AssetManager
 * asset managers}. Some additional utilities are provided for use with {@link com.badlogic.gdx.graphics.g2d .Animation
 * animations} and {@link Color colors}.
 *
...
public final class AssetUtils extends UtilitiesClass {
	...
	/**
	 * Initialize an AssetManager, time will be spent loading whatever initial assets are specified to be immediately
	 * loaded in the parameter. These will be immediately available. This will also generate all TrueType Fonts.
	 *
	 * @param splashTexture
	 ...
	 *
	 * @deprecated Deals only with hardcoded fonts, should just use generators to generate fonts
	 */
	@Deprecated
	...
	/**
	 * Initialize an AssetManager, no initial assets will be loaded. This will also generate all TrueType Fonts.
	 *
	 * @return AssetManager with only the splash texture loaded
	 */
	 ...
	 * asynchronously loads assets using {@link AssetManager#update()}, after each call the provided {@link
	 * FloatConsumer} is called with the current progress of loading provided by {@link AssetManager#getProgress()} and
	 * then yields to the CPU.
	 *
	 * @param manager
	 ...
	 * 	Action after every manager update.
	 */
	public static void ensureLoadAssets(AssetManager manager, Iterable<AssetDescriptor> descriptors,
	...
	/**
	 * Attempts to unload the desired references. Partial completion may occur. <br /> <br /> If, according to LibGDX,
	 * there is more than one reference in the {@link AssetManager#getReferenceCount(String) reference count} then the
	 * unloading fails. This is also what {@link AssetManager#unload(String)} checks before unloading.
	 *
	 * @param manager
	 ...
	 */
	public static TextureRegion[] getAnimationStrip(Texture t, int n, int r, int c, int width, int height,
	                                                Function<TextureRegion, TextureRegion> frameTransform) {
	...
	/**
	 * Create an animation strip (an array of {@link TextureRegion TextureRegions}) from a base texture. It is assumed
	 * the frames start at the top left of the texture.
	 *
	 * @param t
	 * 	Base texture
	 * @param n
	...
	/**
	 * Takes a bit pattern representing an RGB value where the rightmost 3 bytes each represent one component. There is
	 * some loss of precision when the RGB components are converted to a value between 0 and 1. The alpha value is left
	 * unchanged.
	 *
	 * @param hex
